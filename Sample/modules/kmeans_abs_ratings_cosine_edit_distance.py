...
def identify_metaphors_abstractness_cosine_edit_dist(candidates, cand_type, verbose: str) -> MetaphorGroup:
    ...
    try:
        ...
        graph_plot_function( accuracy_confidence_list )
...
def plot_scatter_movingAvg(y):
    # L = [[0, 1, 'f'], [4, 2, 't'], [9, 4, 'afsd']]
    sorted_list_pair = sorted( y, key=itemgetter( 1 ) )
    print( sorted( y, key=itemgetter( 1 ) ) )

    # for pair in sorted_list_pair:
    #     i=0
    #     # i =len(sorted_list_pair)=1
    #     if i+5<=(len(sorted_list_pair)-1):
    #         acc_avg=[sum(sorted_list_pair[i:i+5][0])
    #         Output = [sum( Input[i:i + splits] ) / splits
    #                   for i in range( len( Input ) - splits + 1 )]
    #         conf_avg=sorted_list_pair[i:i+4][1]
    conf_list = []
    acc_list = []
    for pair in sorted_list_pair:
    ...
    acc_counter = [sum( acc_list[i:i + splits] ) / splits
                   for i in range( len( acc_list ) - splits + 1 )]
...
#     counter_conflist = Counter( conf_list )
#     acc_val = []
#     for k, v in counter_acclist.items():
#         if v >= 5:
#             acc_val = acc_val.append( counter_acclist.get( k, 0 ) )
#
#     # splits = 10
#     # conf_counter = [sum( conf_list[i:i + splits] ) / splits
#     #                 for i in range( len( conf_list ) - splits + 1 )]
...
#     y = acc_counter
#
#     plt.scatter( x, y, s=2.4, c="b", alpha=0.8, label="{}_words_average".format( splits ) )
#     plt.xlim( 0.3, 1 )
#     plt.ylim( 0, 1.05 )
#     plt.xlabel( "Confidence" )
#     plt.ylabel( "Avg accuracy - {} words".format( splits ) )
#     plt.legend( loc='lower left' )
#     plt.savefig( "scatter_average_accuracy{}.png".format( int( time.time() ) ), dpi=300,
#                  quality=100 )
#     plt.close()
...
def calc_homogenity_comp_vmeas(user_input_df, candidates):
    ...
    print(
        "homogenity_scr={},vmeasure_scr={},completness_scr={}".format( homogenity_scr, vmeasure_scr, completness_scr ) )
...
graph_plot_function = plot_scatter_movingAvg
